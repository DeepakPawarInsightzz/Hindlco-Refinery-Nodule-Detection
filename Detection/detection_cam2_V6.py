...
logging.basicConfig(filename=os.path.join(code_path, "cam2Detect.log"),filemode='a',format="%(asctime)s - %(name)s - %(levelname)s - %(message)s")
dalogger=logging.getLogger("cam2Detect")
dalogger.setLevel(logging.CRITICAL) #CRITICAL #DEBUG

dataSavePath = "/home/rfserver/insightzz/CSM1/code/DETECTION/DataReview/cam2Data/"
...
imgPath = "/home/rfserver/insightzz/CSM1/code/NUCCTRL_TRANSFER/CAM2/TMP.jpg"
remotePath = "/home/nuc-ctrl/insightzz/code/PROCESSED_IMAGES/CAM2/TMP.jpg"
...
def updateProcessId(processId):
    try:
        ...
        query = f"UPDATE PROCESS_ID_TABLE set PROCESS_ID = {str(processId)} where PROCESS_NAME = 'ALGORITHM_CAM2'"
        cur.execute(query)
        db_update.commit()
        cur.close()
        db_update.close()
...
def get_Auto_info():
    try:
        ...
        print(e)
        dalogger.critical(f"Exception in Auto : {e}")
...
def getimagelink(parentdir):
    try:
        dirname = datetime.datetime.now().strftime('%Y-%m-%d')+"/CAM2"
            ...
            try:
                    ...
                    with open((im2_path), 'rb') as f:
                        ...
                        if f.read() != b'\xff\xd9':
                            ...
                            try:
                                imgsize1 = os.path.getsize(im2_path)
                                if imgsize1 != imgsize2:
                                    im = cv2.imread(im2_path)
                                    image_read = True
                                    imgsize2 = imgsize1
                                else:
                                    image_read = False
                    ...
                    if len(plate_list) > 0:
                        ...
                        if max_x > plate_list[0][7] > min_x:
                            ...
                            for countplate, plate in enumerate(plate_list):
                                ...
                                if len(approx) >= 4:
                                    try:
                                        ...
                                        dalogger.critical(f"Bounding Box : {e}")
                            ...
                            if noduleCount > prevNoduleCount and noduleCount >= number_of_defects:
                                ...
                                print(areaString)
                                #Logging the defect
                                logDefectsDetailsInDBInThread(config_id, "CAM2", imagepath, "nodule", areaString, noduleCount, 1)
                    ...
                    # cv2.imshow("CAM2video", resized)
                    # if cv2.waitKey(1) & 0xFF == ord('q'):
                    #     break

                    cv2.imwrite(imgPath,resized)
                    try:
                        # os.system("scp -i ~/.ssh/id_rsa /home/rfserver2/insightzz/code/NUCCTRL_TRANSFER/CAM2/TMP.jpg nuc-ctrl@169.254.0.28:/home/nuc-ctrl/insightzz/code/PROCESSED_IMAGES/CAM2/TMP.jpg")
                        sftp.put(imgPath, remotePath)
                    except Exception as e:
                        dalogger.critical(f"transfer error : {e}")    # out.release()                
            except Exception as e:
                dalogger.critical(f" In while loop Exception : {e}")
