...
config_path = os.path.join("/home/rfserver/insightzz/CSM1/code/DETECTION/detection_cam1.xml")
import ast
...
logging.basicConfig(filename=os.path.join(code_path, "cam1Detect.log"),filemode='a',format="%(asctime)s - %(name)s - %(levelname)s - %(message)s")
dalogger=logging.getLogger("cam1Detect")
...
imgPath = "/home/rfserver/insightzz/CSM1/code/NUCCTRL_TRANSFER/CAM1/TMP.jpg"
remotePath = "/home/nuc-ctrl/insightzz/code/PROCESSED_IMAGES/CAM1/TMP.jpg"
...
def updateProcessId(processId):
    try:
        ...
        query = f"UPDATE PROCESS_ID_TABLE set PROCESS_ID = {str(processId)} where PROCESS_NAME = 'ALGORITHM_CAM1'"
        cur.execute(query)
        db_update.commit()
        cur.close()
        db_update.close()
        ...
        dalogger.critical(f"Exception in update process id : {e}")    # out.release()
...
def get_configid():
    try:
        ...
        print(e)
        dalogger.critical(f"Exception in get configid: {e}")    # out.release()
...
class maskRCNN:
    ...
    def run_inference(self,img):
        try:
            ...
            dalogger.critical(f"Exception in run inference : {e}")    # out.release()
...
def getimagelink(parentdir):
    try:
        dirname = datetime.datetime.now().strftime('%Y-%m-%d')+"/CAM1"
...
def returnLen(label, plate_list):
    try:
        ...
        yPixByMM = (plateYmax - plateYmin)/1000
        # print(yPixByMM)
        ...
        dalogger.critical(f"Exception in returnLen : {e}")    # out.release()
        return 0,0
        ...
        while True:
            ...
            with open((im1_path), 'rb') as f:
                ...
                if f.read() != b'\xff\xd9':
                    ...
                    try:
                        imgsize1 = os.path.getsize(im1_path)
                        if imgsize1 != imgsize2:
                            im = cv2.imread(im1_path)
                            image_read = True
                            imgsize2 = imgsize1
                        else:
                            image_read = False
            ...
            # print(f"time for inference : {int(time.time()*1000) - t1}")
            plate_list = []
            nodule_list = []
            noCopperList = []
            ...
            if len(plate_list) > 0:
                ...
                if max_x > plate_list[0][7] > min_x:
                    ...
                    try:
                        for count, noCu in enumerate(noCopperList):
                            if plate_list[0][1] < noCu[6] < plate_list[0][2] and plate_list[0][3] < noCu[7] < plate_list[0][4]:
                                ...
                                im = cv2.drawContours(im, noCu[10], -1, (255,255,0), 2)
                    ...
                    for countplate, plate in enumerate(plate_list):
                        ...
                        if len(approx) >= 4:
                            try:
                                ...
                                num_points = 6
                                rightPoints = np.linspace(topRight, bottomRight, num_points, dtype=np.int32)
                                # Get equidistant points on the left side line
                                num_points = 6
                                leftPoints = np.linspace(topLeft, bottomLeft, num_points, dtype=np.int32)
                                # Get equidistant points on the top side line
                                num_points = 6
                                topPoints = np.linspace(topLeft, topRight, num_points, dtype=np.int32)
                                # Get equidistant points on the bottom side line
                                num_points = 6
                                bottomPoints = np.linspace(bottomLeft, bottomRight, num_points, dtype=np.int32)
                                ...
                                areaList = []
                                areaList = tuple(getAreaDictNodule(noduleListRefined, topPoints, bottomPoints, rightPoints, leftPoints))
                                # print(areaList)
                                ...
                                print(e)
                                dalogger.critical(f"puttext : {e}")
                            noduleCount = 0
                            defectSize = []
                            for count, nodule  in enumerate(noduleListRefined):
                                color = tuple(COLORS[count])
                    ...
                    if noduleCount > prevNoduleCount and noduleCount >= number_of_defects:
                        ...
                        #Logging the defect
                        logDefectsDetailsInDBInThread(config_id, "CAM1", imagepath, "nodule", areaString, noduleCount, 1)
                        #creating rejection trigger file
                        if status == "Auto":
                            os.system(f"touch {trigger_file_path}")
                            #print("trigger file generate")              
            ...
            # cv2.imshow("CAM1video", resized)
            # if cv2.waitKey(1) & 0xFF == ord('q'):
            #     break

            cv2.imwrite(imgPath,resized)
            try:
                # os.system("scp -i ~/.ssh/id_rsa /home/rfserver2/insightzz/code/NUCCTRL_TRANSFER/CAM1/TMP.jpg nuc-ctrl@169.254.0.28:/home/nuc-ctrl/insightzz/code/PROCESSED_IMAGES/CAM1/TMP.jpg")
                sftp.put(imgPath, remotePath)
            except Exception as e:
                dalogger.critical(f"transfer error : {e}")    # out.release()                
            # print(f"time : {int(time.time()*1000) - t1}")

        # cv2.destroyAllWindows()
    except Exception as e:
        dalogger.critical(f"Exception in main : {e}")    # out.release()
