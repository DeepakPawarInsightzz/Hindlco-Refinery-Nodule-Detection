...
light_file_path = "/home/rfserver/insightzz/CSM2/code/PLC/light"
...
def main():
    ...
    while True:
        ...
        if os.path.exists(light_file_path) == True and temp1 == False:
            t1 = int(time.time()*1000)
            print("start plc")
            plcObject.startPLC()
            temp1 = True
            temp2 = False
        elif os.path.exists(light_file_path) == False and temp2 == False: 
            print("stop plc")
            plcObject.stopPLC()
            temp2 = True
            temp1 = False
        if os.path.exists(light_file_path) == True and temp1 == True:
           if int(time.time()*1000) - t1 > timer:
                t1 = int(time.time()*1000)
                os.system(f"rm {light_file_path}")
